...
function isassigned(a::Array{T}, i::Int...) where {T}
    @inline
    ii = (_sub2ind(size(a), i...) % UInt) - 1
    @boundscheck ii < length(a) % UInt || return false
    if allocatedinline(T) && (T isa DataType) && datatype_pointerfree(T)
        return true
    else
        return ccall(:jl_array_isassigned, Cint, (Any, UInt), a, ii) == 1
    end
end
