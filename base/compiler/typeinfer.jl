...
function CodeInstance(result::InferenceResult, @nospecialize(inferred_result),
    ...
    return CodeInstance(result.linfo,
        ...
        const_flags, first(valid_worlds), last(valid_worlds), encode_effects(effects), encode_effects(ipo_effects))
end
...
function cache_result!(interp::AbstractInterpreter, result::InferenceResult)
    ...
    if !already_inferred
        ...
        code_cache(interp)[linfo] = CodeInstance(result, inferred_result, valid_worlds,
            ...
            result.ipo_effects, result.ipo_effects)
    end
...
function record_slot_assign!(sv::InferenceState)
    ...
    for i = 1:length(body)
        ...
        if was_reached(sv, i) && isa(expr, Expr) && expr.head === :(=)
            lhs = expr.args[1]
            rhs = expr.args[2]
